...
graph_t* graph_init()
{
    ...
    int vertex_set_offset = offsetof(vertex_t, set);
    int edges_set_offset = offsetof(edge_t, set);
    ...
    graph->vertices_sets = create_disjoint_sets(vertex_set_offset, print_vertex_set);

    graph->edges_size = 1;
    graph->edges = malloc(sizeof(edge_t*) * graph->edges_size);
    memset(graph->edges, 0, sizeof(edge_t*) * graph->edges_size);
    graph->edges_sets = create_disjoint_sets(edges_set_offset, print_edge_set);
...
void graph_delete(graph_t* graph)
{
    ...
    free(graph->vertices);
    delete_disjoint_sets(graph->vertices_sets);

    free(graph->edges);
    delete_disjoint_sets(graph->edges_sets);
...
vertex_t* vertex_malloc(int index, int id, int size)
{
    ...
    // Set set
    vertex->set.parent = NULL;
    vertex->set.rank = 0;
    vertex->set.next = NULL;
    vertex->set.prev = NULL;
...
void add_vertex(graph_t* graph, int index, int id)
{
    ...
    add_set_to_disjoint_sets(graph->vertices_sets, vertex);
    graph->num_vertices++;
...
void add_directed_edge(graph_t* graph, int source, int destination, int weight)
{
    ...
    add_set_to_disjoint_sets(graph->edges_sets, edge);
...
graph_t* copy_graph(graph_t* graph)
{
    ...
    {
        int index = graph->vertices[i]->index;
        int id = graph->vertices[i]->id;
        add_vertex(copy, index, id);
    }
...
void sort_given_edges_min(edge_t** edges, int size)
{
    qs_t min_sorted_edges;
    qs_init(&min_sorted_edges, edges, size, min_compare, print_edges);

    quicksort(&min_sorted_edges, 0, size - 1);

    return;
}

void sort_given_edges_max(edge_t** edges, int size)
{
    qs_t max_sorted_edges;
    qs_init(&max_sorted_edges, edges, size, min_compare, print_edges);

    quicksort(&max_sorted_edges, 0, size - 1);
