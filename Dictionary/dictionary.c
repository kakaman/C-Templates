...
void destroy_entry(dictionary_entry_t* entry)
{
    if(entry == NULL)
    {
        return;
...
void recursive_destroy(dictionary_entry_t* current)
{
    if(current == NULL)
    {
        return;
    ...
    {
        recursive_destroy(current->left);
    }
    else if(current->right != NULL)
    {
        recursive_destroy(current->right);
    }
...
void dictionary_destroy_all(dictionary_t* dictionary)
{
    ...
    // Take lock. Call recursive_destroy to remove all elements. Unlock.
    pthread_mutex_lock(&dictionary->mutex);
    recursive_destroy(dictionary->root);
    pthread_mutex_unlock(&dictionary->mutex);

    // Remove dictionary memory.
